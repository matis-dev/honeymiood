...
* **Cargo.site's hosting ceiling — what could not be done and why**: this site is hosted on Cargo.site, which was verified live (curl against `honeymiood.com`) rather than assumed:
  ...
  - Cargo exposes exactly one global `<head>`-adjacent surface (a site-wide "meta tags" string and one preview image) and no per-page `<head>` at all. Per-page canonical URLs, `hreflang` alternates and geo meta tags therefore only exist in the standalone build's `<head>` (`tools/build-site.py`); on the Cargo-hosted site, JSON-LD is instead appended to each page's body (valid per schema.org and read by Google/Perplexity there), and the geo meta tags need one manual paste into Cargo's site-wide meta field (see DEPLOY.md §6).
...
* **Ograniczenia hostingu Cargo.site — czego nie dało się zrobić i dlaczego**: strona jest hostowana na Cargo.site, co zweryfikowano na żywo (curl na `honeymiood.com`), a nie założono z góry:
  ...
  - Cargo udostępnia dokładnie jedną globalną powierzchnię zbliżoną do `<head>` (jeden, wspólny dla całej strony ciąg "meta tags" i jedno zdjęcie podglądu) i żadnego `<head>` per-strona. Adresy kanoniczne, warianty `hreflang` i tagi geo per-strona istnieją więc tylko w samodzielnej wersji strony (`tools/build-site.py`); na stronie hostowanej przez Cargo dane JSON-LD są zamiast tego dołączane do treści (`body`) każdej strony (zgodnie ze specyfikacją schema.org i czytane stamtąd przez Google/Perplexity), a tagi geo wymagają jednorazowej, ręcznej wklejki do globalnego pola meta na Cargo (patrz DEPLOY.md §6).
