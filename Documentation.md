...
2. **Per-page `<head>` content doesn't reach Cargo.** Cargo exposes one global preview image and one global meta-tags string, no per-page `<head>`. Canonical URLs, `hreflang` alternates and geo meta tags therefore exist only in the standalone build; on the live Cargo site, JSON-LD is instead appended to each page's `<body>` (`tools/build-cargo.py`), which is valid per schema.org and is where Google/Perplexity read it from regardless.
